...
export default function SignInModal({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res
      if (mode === 'signup') {
        res = await registerUser({ fullName, email, password, specialty })
      ...
      const authUser = {
        ...
        displayName: res.user?.fullName || fullName || email.split('@')[0],
        uid: res.user?.id || 'user-' + Date.now(),
        paid: res.user?.paid || false,
        specialty: res.user?.specialty || specialty,
      }

      if (onSuccessfulAuth) {
        onSuccessfulAuth(authUser)
      }
      ...
      console.warn('Authentication response note:', err.message)
      if (err.message.includes('offline') || err.message.includes('Failed to fetch')) {
        ...
        setSuccessMessage(
          mode === 'signup'
            ? `Account registered locally for ${fullName || 'Coder'}! (Local Session)`
            : 'Welcome back! (Local Session)'
        )
        if (onSuccessfulAuth) onSuccessfulAuth(fallbackUser)
        setTimeout(() => {
          setSuccessMessage(null)
          onClose()
        }, 1000)
          ...
          <div className="min-h-full flex items-start sm:items-center justify-center p-3 sm:p-6 py-8 sm:py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10 items-center">
              ...
              <div className="md:col-span-7 flex flex-col justify-center">
                ...
                {errorMessage && (
                  <div className="mb-4 rounded-xl bg-red-500/10 border border-red-500/30 p-3 text-left flex items-center gap-2 text-xs font-semibold text-red-600">
