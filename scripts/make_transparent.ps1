...
{
    public static void RemoveBackground(string srcPath, string destPath)
    {
        using (Bitmap src = new Bitmap(srcPath))
        {
            ...
            // Sample 4 corners average color
            ...
            while (queue.Count > 0)
            {
                ...
                // Threshold for background (white/off-white background)
                if (diff < 35 || (brightness > 230 && diff < 55) || brightness > 250)
                {
                    isBg[idx] = true;
            ...
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    ...
                    else
                    {
                        ...
                        // Check edge for smooth antialiasing
                        bool isEdge = false;
...
foreach ($item in $images) {
    Write-Host "Converting $($item.Src) -> $($item.Dest)"
    [ImageProcessor]::RemoveBackground($item.Src, $item.Dest)
    Write-Host "Success: $($item.Dest)"
}
